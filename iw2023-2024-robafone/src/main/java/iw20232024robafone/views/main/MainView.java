...
public class MainView extends VerticalLayout {
    public MainView(SecurityService securityService, EmployeeService employeeService, ClientService clientService) {
        ...
        if (securityService.getAuthenticatedUser().getAuthorities().toString().equals("[ROLE_CLIENT]")) {
            add(new FrontOfficeMainView(securityService));
        }
    }
}